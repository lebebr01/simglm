...
test_that('number of columns, categorical and continuous', {
  sim_arguments <- list(
    ...
    fixed = list(
      turnover = list(var_type = 'continuous', mean = 10, sd = 3),
      type = list(
        var_type = 'factor',
        levels = c('A', 'B', 'C'),
        prob = c(0.2, 0.6, 0.2)
...
test_that('knot sim', {
  sim_args <- list(
    ...
    knot = list(age_knot = list(variable = 'age', knot_locations = 50)),
    sample_size = 250,
    error = list(variance = 10),
    reg_weights = c(2, 0.5, 1.5)
  )
  ...
  expect_equal(
    length(unique(simulate_fixed(data = NULL, sim_args = sim_args)[[
      'age_knot'
    ]])),
    2
  )
...
test_that("Ceiling and floor effects", {
  sim_arguments <- list(
    ...
    fixed = list(
      turnover = list(var_type = 'continuous', mean = 10, sd = 3, ceiling = 15)
    ),
    error = list(variance = 0.1),
    sample_size = 100
  )
  ...
  sim_arguments <- list(
    formula = y ~ 1 + turnover,
    fixed = list(
      turnover = list(var_type = 'continuous', mean = 10, sd = 3, floor = 8)
    ),
    error = list(variance = 0.1),
    sample_size = 100
  )
