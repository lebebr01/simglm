...
test_that('random missing', {
  ...
  expect_true(anyNA(data_w_missing[['y_missing']]))
  expect_false(anyNA(data_w_missing[['y']]))
  expect_equal(
    prop.table(table(is.na(data_w_missing[['y_missing']])))[[2]],
    0.25,
    tolerance = 0.02
  )
})
...
test_that('dropout missing', {
  ...
  expect_true(anyNA(data_w_missing[['missing_y']]))
  expect_false(anyNA(data_w_missing[['y']]))
  expect_equal(
    prop.table(table(is.na(data_w_missing[['missing_y']])))[[2]],
    0.226,
    tolerance = 0.02
  )

  prop_missing <- prop.table(table(
    is.na(data_w_missing[['missing_y']]),
    data_w_missing[['time']]
  ))[2, ]
...
test_that("missing at random", {
  ...
  sim_arguments <- list(
    ...
    missing_data = list(
      new_outcome = 'y_missing',
      miss_cov = 'weight',
      mar_prop = seq(from = 0, to = 0.9, length.out = 200),
      type = 'mar'
    ),
    sample_size = list(level1 = 10, level2 = 20)
  )
  ...
  prop_missing <- table(
    is.na(data_w_missing[['y_missing']]),
    cut(data_w_missing[['weight']], breaks = 10)
  )[2, ] /
...
test_that("mar small", {
  ...
  prop_missing <- addmargins(table(
    simulated_data$treat,
    is.na(simulated_data$y_missing)
  ))[, 2] /
    addmargins(table(simulated_data$treat, is.na(simulated_data$y_missing)))[,
      3
    ]

  expect_equal(
    as.numeric(round(prop_missing[1:3], 2)),
    c(0.1, 0.2, 0.4),
    tolerance = 1,
  )
})
