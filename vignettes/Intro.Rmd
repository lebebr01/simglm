...
The rest of the arguments are pretty straightforward, **n** is the sample size, **error_var** is the error variance, **rand_gen** is the distribution of the residuals, and finally in the function call itself, **data.str** must be "single" in this instance to reflect a single level regression.
...
n = n, error_var = error_var, rand_gen = rand_gen, data.str = "single", fact.vars = fact.vars)
```
...
temp.long <- sim_reg(fixed = fixed, random = random, 
                     ...
                     rand.dist = rand.dist, rand_gen = rand_gen,
...
The other new terms needed are straightforward, **p** is the within cluster sample size (i.e. how many repeated measurements), **randCor** is the correlation between the random effects, **rand.dist** is the distribution of the random effects - see the help file for details on options here, and lastly **data.str** is now "long" for longitudinal data.
