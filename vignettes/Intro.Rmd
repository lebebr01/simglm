...
error_var <- 3
with_err_gen = 'rnorm'
temp_single <- sim_reg(fixed = fixed, fixed.param = fixed.param, cov.param = cov.param,
...
```{r simpregmod}
summary(lm(sim.data ~ 1 + act + diff + numCourse + act:numCourse, data = temp_single))
```
...
error_var <- 3
with_err_gen = 'rnorm'
temp_single_o <- sim_reg(fixed = fixed, fixed.param = fixed.param, 
