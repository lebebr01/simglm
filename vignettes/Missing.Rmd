...
with_err_gen <- 'rnorm'
data_str <- "long"
temp_long <- sim_reg(fixed, random, random3 = NULL, fixed.param, random_param, random_param3 = NULL,
 cov.param, k = NULL, n, p, error_var, with_err_gen, data_str = data_str)

# simulate missing data
temp_long_miss <- missing_data(temp_long, miss_prop = .25, type = 'dropout', clust_var = 'clustID')
head(temp_long_miss)
```
...
prop.table(table(is.na(temp_long_miss$sim.data2)))
```
...
error_var <- 3
with_err_gen <- 'rnorm'
temp_single <- sim_reg(fixed = fixed, fixed.param = fixed.param,
                       cov.param = cov.param,
...
```{r marmisscheck}
table(tmp_single_miss$miss_prop,is.na(tmp_single_miss$sim.data2))
```
...
```{r mcarmissverify}
prop.table(table(is.na(tmp_single_miss$sim.data2)))
```
<!--
